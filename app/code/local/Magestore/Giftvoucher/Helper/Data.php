...
//        );
    ...
    public function getFullGiftVoucherOptions() {
        return array(
            ...
            'notify_success' => $this->__('Notify when recipient receipt giftcard.')
        );
    }
    ...
    public static function uploadImage($type) {
        ...
        if (isset($_FILES[$type]['name']) && $_FILES[$type]['name'] != '') {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('giftvoucher')->__('Input file format not correct. Allow type: jpg, jpeg, gif, png'));
            }
        }
        return $image;
    }
    ...
    public function customResizeImage($imagePath, $imageName, $imageType) {
    ...
    public function createBarcode($gift_code) {
        ...
        imagepng($resource, 'media/giftvoucher/template/barcode/' . $gift_code . '.png');
