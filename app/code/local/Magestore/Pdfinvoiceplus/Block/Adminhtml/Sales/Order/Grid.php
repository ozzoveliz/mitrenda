...
//        );
         $this->getMassactionBlock()->addItem(
            ...
            'url' => $this->getUrl('pdfinvoiceplus/adminhtml_order/printmassorder') //this should be the url where there will be mass operation
                )
        );
         $this->getMassactionBlock()->addItem(
            'pdfinvoice', array('label' => $this->__('Print Invoices via PDF Invoice+'),
            'url' => $this->getUrl('pdfinvoiceplus/adminhtml_invoice/printmassinvoice') //this should be the url where there will be mass operation
                )
        );
          $this->getMassactionBlock()->addItem(
            'pdfcreditmemo', array('label' => $this->__('Print Credit Memos via PDF Invoice+'),
            'url' => $this->getUrl('pdfinvoiceplus/adminhtml_creditmemo/printmasscreditmemo') //this should be the url where there will be mass operation
                )
        );
    }
