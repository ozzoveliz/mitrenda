    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($data['is_select_design']) && $data['is_select_design'] && $data['system_template_id'] != '') {
                ...
                $data["invoice_html"] = utf8_encode($blockSelect->toHtml());
                $blockSelect->setTemplate('pdfinvoiceplus/loadtemplate/' . $templateCode . '/order.phtml');
                $data["order_html"] = utf8_encode($blockSelect->toHtml());
//                echo $data["order_html"];die('1');
                $blockSelect->setTemplate('pdfinvoiceplus/loadtemplate/' . $templateCode . '/creditmemo.phtml');
                $data["creditmemo_html"] = utf8_encode($blockSelect->toHtml());
            }
            //bug not add html when null
//            $order_html = $data['order_html'];
//            $invoice_html = $data['invoice_html'];
//            $creditmemo_html = $data['creditmemo_html'];
            //unset($data['order_html'],$data['invoice_html'],$data['creditmemo_html']);
            $model->addData($data)
...
//            if($order_html){
//                $model->setOrderHtml($order_html);
//            }
//            if($invoice_html){
//                $model ->setInvoiceHtml($invoice_html);
//            }
//            if($creditmemo_html){
//                $model ->setCreditmemoHtml($creditmemo_html);
//            }
            ...
            try {
                ...
                if (isset($editdesign) && $editdesign != null) {
                    if ($editdesign == 'order') {
                        $this->_redirect('pdfinvoiceplus/adminhtml_design/editOrder', array('id' => $model->getId()));
                        return;
                    } else if ($editdesign == 'invoice') {
                        $this->_redirect('pdfinvoiceplus/adminhtml_design/editInvoice', array('id' => $model->getId()));
                        return;
                    } else {
                        $this->_redirect('pdfinvoiceplus/adminhtml_design/editCreditmemo', array('id' => $model->getId()));
                        return;
                    }
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
