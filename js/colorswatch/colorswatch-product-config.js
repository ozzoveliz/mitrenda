...
Product.Config.prototype.sendAjaxRequest = function(element, selectedOptionId, event) {
    ...
    var w = $$('.cloud-zoom')[0].getWidth();
    var parent = $$('.cloud-zoom')[0];
    parent.insert(format('<div style="width:%0px;position:absolute;top:75%;left:%1px;text-align:center" class="cloud-zoom-loading" >Loading...</div>', w / 3, (w / 2) - (w / 6)));
    var childLoading = parent.select(':last-child')[0]
    childLoading.setStyle({'opacity':0.5});
